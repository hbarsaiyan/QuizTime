...
var marked = function () {
	...
	for (var i = 1; i <= nos.length; i++) {
		if (
			...
			data[i].status == SUBMITTED_BOOKMARKED
		) {
			count++;
			$('#question-list').empty();
			ques_grid();
...
var ques_grid = function () {
	...
	for (var i = 1; i <= nos.length; i++) {
		...
		$('#question-list').append(
			'<div class="col-sm-2"><button class="btn btn-primary" style="background-color:' +
				color +
				'; color:white;"><div class="question" style="background-color:' +
				color +
				'; color:white;">' +
				j +
				'</div></div></div>'
...
function funSubmitExam() {
	...
	Swal.fire({
		...
		icon: 'warning',
		html:
			'<table><tr><td>TOTAL QUESTIONS:</td><td>' +
			nos.length +
			'</td></tr><tr><td>ATTEMPTED:</td><td>' +
			count +
			'</td></tr><tr><td>REMAINING:</td><td>' +
			remaining +
			'</td></tr></table>',
